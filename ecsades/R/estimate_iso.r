...
#' 
#' @param obj the fitted joint distribution object as generated by function \code{\link{fit_ht}}
...
#' 
#' @export
estimate_iso = function(obj, output_rp, n_point=100){
  
  if(class(obj)=="ht"){
    
    res = .estimate_iso_from_ht(
      ht = obj, output_rp = output_rp,
      n_point = n_point)
    
  }else if(class(obj)=="wln"){
    
    res = .estimate_iso_from_wln(
      wln = obj, output_rp = output_rp,
      n_point = n_point)
    
  }else if("data.table" %in% class(obj)){
