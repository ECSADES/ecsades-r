...
#' 
#' @param ref_hs,ref_tp the wave height and wave period for the reference point(s). When either or both
#' is missing, the default reference points are the \code{output_rp}-year return levels for the wave heights,
#' and the corresponding conditional medians for the wave periods. See details.
#' 
#' @param knn the number of nearest neighbours (by \code{hs}) to calculate the conditional median of
#' \code{tp|hs}. The default value is 100.
#' 
#' @details
...
#' and \code{tp} axes respectively, and facing away from the reference point.
#' 
...
estimate_gje = function(
  ...
  output_rp, n_point = 100, ref_hs = NULL, ref_tp = NULL, knn=100){
  ...
  if(is.null(ref_hs) || is.null(ref_tp)){
    ...
    rl_hs = approx(
      ...
      xout = log(output_rp))$y
    ref_tp = c()
    for(i in 1:length(output_rp)){
      ref_tp[i] = sample_data[sort.list(abs(hs-rl_hs[i]))[1:knn], median(tp)]
    }
